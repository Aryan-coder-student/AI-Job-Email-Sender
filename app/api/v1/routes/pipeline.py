...
from app.modules.company_imports import preview_company_import, preview_company_import_from_url
...
async def preview_pipeline_companies(
    companies: UploadFile | None = File(default=None),
    companies_url: str | None = Form(default=None),
    header_row: int = Form(default=1),
    sheet_names: str = Form(default=""),
    max_rows: int | None = Form(default=None),
) -> dict[str, Any]:
    try:
        config = ExcelParserConfig(
            header_row=header_row,
            sheet_names=tuple(s.strip() for s in sheet_names.split(",") if s.strip()) if sheet_names else (),
            max_rows=max_rows,
        )
        return await _preview_company_source(
            companies=companies,
...
async def create_pipeline_run(
    ...
    active_companies_url = _clean_company_url(companies_url)
    run = store.create_run(
        config={
            ...
            "companies_url": active_companies_url,
        },
...
@router.put("/{run_id}/drafts")
def update_pipeline_drafts(run_id: str, payload: DraftUpdateRequest) -> dict[str, Any]:
    drafts = get_run_store().update_draft(run_id, payload.model_dump(exclude_unset=True))
    if drafts is None:
        raise not_found("Run or drafts not found.")

    return drafts


@router.post("/{run_id}/drafts/enqueue")
def enqueue_pipeline_drafts(run_id: str) -> dict[str, Any]:
    drafts = get_run_store().enqueue_draft(run_id)
    if drafts is None:
        raise not_found("Run or drafts not found.")
...
def _selected_companies(
    *,
    ...
    active_url = _clean_company_url(url)
    if active_url:
        return _parse_company_url(url=active_url)

    return []


async def _preview_company_source(
    *,
    companies: UploadFile | None,
    companies_url: str | None,
    config: ExcelParserConfig | None = None,
) -> dict[str, Any]:
    active_url = _clean_company_url(companies_url)
    if active_url:
        return preview_company_import_from_url(
            url=active_url,
    ...
    if companies is None:
        raise HTTPException(
            status_code=400,
            detail="Companies file or Google Sheet URL is required.",
        )
...
def _parse_company_url(*, url: str) -> list[dict[str, Any]]:
    try:
        preview = preview_company_import_from_url(
            url=_clean_company_url(url) or url,
            output_dir=get_run_store().output_dir,
        )
    except (InvalidExcelError, UnicodeDecodeError, ValueError) as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
...
def _clean_company_url(url: str | None) -> str | None:
